...
temp_csv1 = os.path.join(os.path.dirname(__file__), 'csv_files', 'temp_csv1.csv')
...
dns_fw_dm = [{'stesworld.com': [('10.30.10.81', '@', '00:10:00'),
             ...
             {'stesworld.co.uk': [('172.16.48.5', 'computer46', '01:00:00')]}]
dns_rv_dm =  [{'10.10.10.in-addr.arpa': [('53', 'computer53.stesworld.com.'),
                                         ('45', 'computer45.stesworld.com.')]},
              {'20.20.20.in-addr.arpa': [('44', 'computer44.stesworld.com.')]},
              {'16.172.in-addr.arpa': [('7.48', 'computer47.stesworld.co.uk.')]}]

def test_verify_csv_vs_dns():
    test = Dns(dns_svr, user, password, csv_dns_fw_dm, csv_dns_rv_dm)
    x = test.verify_csv_vs_dns(dns_fw_dm, dns_rv_dm)

    assert x['len_csv'] == '8/4', 'Error with detected number of used reservations (CSV vs DHCP server)'
    assert x['used_fqdn'] == [('stesworld.co.uk', 'computer46.stesworld.co.uk'),
    ...
    assert x['missing_fqdn'] == [('stesworld.com', 'computer44.stesworld.com'),
...
def test_create_new_csv():
    test = Dns(dns_svr, user, password, csv_dns_fw_dm, csv_dns_rv_dm)
    type = 'add'
    x = test.create_new_csv(type, temp_csv)
